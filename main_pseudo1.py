...
base_dir = '{}/{}_{}_{}_{}_PL_cut{}'.format(opt.save_dir, opt.dataset, opt.model, opt.noise_type, int(opt.noise*100), int(opt.cut*100))

if opt.load_dir: 
	assert os.path.isdir(opt.load_dir)
	opt.save_dir = opt.load_dir
else: 	
	opt.save_dir = '{}_pseudo1'.format(base_dir)		  		   		
opt.pretrained = '{}/checkpoint_epoch719.pth.tar'.format(base_dir)
try:
...
##
# output: inds_filt, inds_left
ckpt = torch.load(opt.pretrained)
train_preds_hist = ckpt['train_preds_hist']

preds = train_preds_hist.mean(1)[torch.arange(len(trainset)), np.array(trainset.imgs)[:,1].astype(int)]
nl_ratio = float((preds<opt.cut).sum()) / float(len(preds))
logger.info('nl_ratio: %f'%(nl_ratio))

inds = np.argsort(preds)
inds_filt = inds[:int(len(trainset)*nl_ratio)]
inds_left = inds[int(len(trainset)*nl_ratio):]
logger.info(float( len(np.intersect1d(inds_filt, inds_noisy)) ) / float(len(inds_noisy)))
logger.info(float( len(np.intersect1d(inds_filt, inds_noisy)) ) / float(int(len(trainset)*nl_ratio)))
##

weight = torch.FloatTensor(num_classes).zero_() + 1.
for i in range(num_classes):
	weight[i] = (torch.from_numpy(np.array(trainset.imgs)[:,1].astype(int))[inds_left] == i).sum()
...
for epoch in range(epoch_resume, opt.max_epochs):
	...
	for i, data in enumerate(trainloader, 0):
		...
		labels[ np.intersect1d(np.asarray(index), inds_filt, return_indices=True)[1] ] = -100
	...
	label_updated = (torch.max(train_soft_labels, -1)[1]).numpy().tolist()
	logger.info('Label update status...noisy: %d/%d, clean: %d/%d'
		%((np.array(label_updated)[inds_noisy] != np.array(clean_labels)[inds_noisy]).sum(), len(inds_noisy),
		  (np.array(label_updated)[inds_clean] != np.array(clean_labels)[inds_clean]).sum(), len(inds_clean)))
    ###############################################################################################
