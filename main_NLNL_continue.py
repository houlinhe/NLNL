...
def main():
	...
	if opt.load_dir: 
		...
		opt.save_dir = '{}/{}'.format(opt.save_dir, "NLNL")
	try:
	...
	data = data.drop(data[data['Finding Labels'].str.contains("\|") == True].index)
	...
	with open("./train_df.csv") as f:
		train_data_filenames = f.read().splitlines() 
		train_data_filenames = [name.split('\t')[1] for name in train_data_filenames]
	with open("./test_df.csv") as f:
		test_data_filenames = f.read().splitlines()
		test_data_filenames = [name.split('\t')[1] for name in test_data_filenames]

	all_sample_size = 12000
	all_sample_weight = 0.18
	...
	# train_df.to_csv('train_df.csv', sep='\t', encoding='utf-8', header='true', columns=['Image Index'])
	# test_df.to_csv('test_df.csv', sep='\t', encoding='utf-8', header='true', columns=['Image Index'])
	...
	labels = test_df['disease_vec'].values # Double -> Float
