...
def main():
	...
	if opt.load_dir: 
		...
		opt.save_dir = '{}/{}'.format(opt.save_dir, "NLNL3")
	try:
	...
	data_no_temp = data[data['Finding Labels'] == "No Finding"]
	...
	train_df = data_no_temp.sample(train_each_sample)
	test_df = data_no_temp_test.sample(test_each_sample)

	data_no_temp = data[data['Finding Labels'] == "Effusion"]
	data_no_temp_test = data_no_temp[data_no_temp['Image Index'].isin(test_data_filenames)]
	data_no_temp = data_no_temp[data_no_temp['Image Index'].isin(train_data_filenames)]
	train_df = pd.concat([train_df, data_no_temp.sample(train_each_sample)], ignore_index=True, sort=False)
	test_df = pd.concat([test_df, data_no_temp_test.sample(test_each_sample)], ignore_index=True, sort=False)
